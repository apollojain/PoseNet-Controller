...
def build_network(image, layers, variables):
    ...
    def _conv_to_output(mobile_net_output, output_layer_name):
        w = tf.nn.conv2d(
            mobile_net_output, _weights(output_layer_name), [1, 1, 1, 1], padding="SAME"
        )
    ...
    def _conv(inputs, stride, block_id):
        return tf.nn.relu6(
            tf.nn.conv2d(
                inputs, _weights("Conv2d_" + str(block_id)), stride, padding="SAME"
            )
    ...
    with cg.as_default():
        ...
        with tf.Session() as sess:
            ...
            image_ph = tf.placeholder(
                tf.float32, shape=[1, None, None, 3], name="image"
            )
            ...
            save_path = os.path.join(
                model_dir, "checkpoints", "model-%s.ckpt" % chkpoint
            )
            ...
            if check and os.path.exists("./images/tennis_in_crowd.jpg"):
                ...
                input_image = _read_imgfile(
                    "./images/tennis_in_crowd.jpg", width, height
                )
                ...
                ) = sess.run(outputs, feed_dict={image_ph: input_image})
